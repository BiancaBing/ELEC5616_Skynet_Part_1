    ...
    def send(self, data, crypto=True, nonce=None):
        if crypto and (nonce is not None):
            if self.cipher:
                ...
                md5_object = bytes(bytearray(t) + bytearray(encrypted_data) + bytearray(nonce))
                md5 = hmac.new(self.shared_hash, md5_object, hashlib.md5).hexdigest().encode("ascii")
                sending = bytes(bytearray(t) + bytearray(md5) + bytearray(encrypted_data) + bytearray(nonce))
