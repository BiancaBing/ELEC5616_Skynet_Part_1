...
class StealthConn(object):
    ...
    def send(self, data):
        if self.cipher:
            ...
            pkt_len = struct.pack('HHH', len(encrypted_data),len(md5),len(t))
    ...
    def recv(self):
        ...
        if self.cipher:
            ...
            all_data = bytearray(self.conn.recv(data_len + md5_len + time_len))
            md5_received = bytes(all_data[time_len:md5_len + time_len])
            encrypted_data = bytes(all_data[time_len + md5_len:])
